...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("Logs/Token_Train_AcHemo.log", encoding='utf-8'),
...
# ---------------------------------------------------------------------------
MAX_LEN = 50        # longueur max peptide (truncate/pad)
ESM_BATCH = 16      # séquences par batch ESM2
TRAIN_BATCH = 64
EPOCHS = 200
LR = 3e-4
PATIENCE_ES = 15
PATIENCE_LR = 8
NPY_PATH = 'dataset_train_esm2_t33_650M_AcHemo_token_level.npy'
MODEL_PATH = 'ModelCNN_PROTEOGEN_V3/AcHemo_PROTEOGEN_model.keras'
...
dataset = pd.read_excel('Train_AcHemo.xlsx', na_filter=False)
...
if os.path.exists(NPY_PATH):
    X = np.load(NPY_PATH)
    if len(X) != len(sequence_list):
        logging.warning(
            f"Cache .npy périmé : {len(X)} embeddings vs {len(sequence_list)} séquences dans le dataset "
            f"— suppression et recalcul forcé"
        )
        os.remove(NPY_PATH)
        X = esm_embeddings_batch(esm_model, alphabet, sequence_list)
        np.save(NPY_PATH, X)
        logging.info(f"Embeddings recalculés et sauvegardés : {NPY_PATH} — shape {X.shape}")
    else:
        logging.info(f"Embeddings existants chargés : {NPY_PATH} — shape {X.shape}")
else:
...
assert len(X) == len(y), f"ERREUR CRITIQUE : len(X)={len(X)} != len(y)={len(y)} — dataset et cache désynchronisés"
...
# Conv1D scan des positions acides aminés → sémantiquement correct
# ---------------------------------------------------------------------------
def build_cnn(input_shape=(MAX_LEN, 1280)):
    inp = Input(shape=input_shape)

    # Multi-scale : tripeptides (k=3), pentapeptides (k=5), heptapeptides (k=7)
    ...
    # Bloc résiduel avec BN sur skip
    ...
    return Model(inputs=inp, outputs=x, name='ESM_CNN_AcHemo')
...
# Courbe ROC
# ---------------------------------------------------------------------------
def plot_roc_curve(y_true, proba, label, save_path):
    ...
    ax.scatter(fpr[opt_idx], tpr[opt_idx], marker='o', color='red',
               label=f'Seuil optimal = {thresh[opt_idx]:.3f}', zorder=5)
    ...
    plt.close(fig)
    logging.info(f"Courbe ROC sauvegardée : {save_path}")
...
for fold, (tr_idx, val_idx) in enumerate(kf.split(X_train, y_train)):
    ...
    proba = saved_model(X_val, training=False).numpy()[:, 1]
    y_true = y_val
...
proba_test = final_model(X_test, training=False).numpy()[:, 1]
...
plot_roc_curve(y_test, proba_test, "AcHemo", "Plots/ROC_AcHemo.png")